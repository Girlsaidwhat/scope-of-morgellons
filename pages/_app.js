// pages/_app.js
// Build 36.167_2025-09-02
import React, { useEffect, useRef, useState } from "react";
import "../styles/globals.css";
import { useRouter } from "next/router";
import { createClient } from "@supabase/supabase-js";

export const BUILD_VERSION = "Build 36.167_2025-09-02";
...
class ErrorBoundary extends React.Component {
  ...
  static getDerivedStateFromError(error) {
    ...
    let stackTop = "";
    try {
      const s = error?.stack ? String(error.stack) : "";
        ...
        >
          ...
          >
            ...
            <p style={{ opacity: 0.9, marginBottom: 12 }}>
              Something hiccuped. Reload to try again, or use the menu above.
            </p>
            <div
              style={{
                textAlign: "left",
            ...
            >
              <div>
                <strong>Error:</strong> {this.state.msg || "(no message)"}{" "}
              </div>
              {this.state.stackTop ? (
                <div style={{ marginTop: 6 }}>
                  <strong>At:</strong> {this.state.stackTop}
                </div>
              ) : null}
            </div>
            <div style={{ marginTop: 12, opacity: 0.8, fontSize: 12 }}>
              This diagnostic block is temporary; we’ll remove it after the fix.
            </div>
          </div>
        </main>
      );
...
function ExplorePanel() {
  ...
  useEffect(() => {
    const sync = () => {
      const spanW =
        titleSpanRef.current?.getBoundingClientRect?.().width || CONTENT_MAX;
      ...
      >
        ...
        >
          ...
          {menuOpen ? (
            ...
            >
              <a role="menuitem" href="/about" style={linkMenu}>
                About
              </a>
              <a role="menuitem" href="/news" style={linkMenu}>
                News
              </a>
              <a role="menuitem" href="/resources" style={linkMenu}>
                Resources
              </a>
            </nav>
          ) : null}
...
/* ---------- Carousel (even wrap; ~0.8s pause between columns) ---------- */
function CarouselRow({ maxWidth = 560 }) {
  ...
  if (urls.length >= 2) {
    for (let c = 0; c < 3; c++)
      if (cols[c].length < 2)
        cols[c] = [...cols[c], urls[(c + 1) % urls.length]];
  }

  if (!urls.length) return null;

  // Gentle fade with short pause; perfectly even wrap
  const FADE_MS = 2800;
  const HOLD_MS = 8000;
  const T = FADE_MS + HOLD_MS; // 10800
  const delays = [0, Math.round(T / 3), Math.round((2 * T) / 3)]; // 0, 3600, 7200
...
export default function MyApp({ Component, pageProps }) {
  ...
  const isResetPath =
    router?.pathname?.startsWith?.("/auth/reset") || false;
