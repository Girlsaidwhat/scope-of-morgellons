...
export const BUILD_VERSION = "Build 36.19b_2025-08-21";
...
export default function MyApp({ Component, pageProps }) {
  ...
  // Remove legacy build lines & hide any Build text in the top stripe, everywhere
  useEffect(() => {
    const badge = document.getElementById("global-build-badge");
    const versionRe = /Build\s+[\w.\-]+(?:[_-]\d{4}-\d{2}-\d{2})?/i;
    ...
    function scrubTextNodes(root) {
      ...
      while (walker.nextNode()) {
        ...
        if (versionRe.test(n.nodeValue)) toPatch.push(n);
      }
      for (const n of toPatch) {
        n.nodeValue = n.nodeValue.replace(versionRe, "").replace(/\(\s*\)/g, "").replace(/\s{2,}/g, " ").trim();
      }
      root.querySelectorAll?.("[data-build-line]").forEach((el) => el.remove());
    }

    function hideTopStripeBuilds() {
      const allEls = Array.from(document.body.querySelectorAll("*"));
      for (const el of allEls) {
        if (!el || el === badge || (badge && el.contains?.(badge))) continue;
        const text = (el.textContent || "").trim();
        if (!/Build/i.test(text)) continue;

        const rect = el.getBoundingClientRect?.();
        if (!rect) continue;

        // Anywhere in the top stripe (left or right) counts
        const nearTopStripe = rect.top >= 0 && rect.top < 220;

        if (nearTopStripe) {
          el.setAttribute("data-build-hidden", "true");
    ...
    // On route changes
    const onRouteDone = () => scrubAll();
    router.events.on("routeChangeComplete", onRouteDone);

    // Observe late changes (and re-scrub)
